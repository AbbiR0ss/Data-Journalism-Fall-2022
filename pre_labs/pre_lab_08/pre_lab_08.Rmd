...
**Answer**
There are 20 columns and 25 rows.
...
**Answer**
The first row of the code is simply creating a new variable. The next line is arranging the TOTAL_DIFF column from largest to smallest. The third line is where it narrows it down so that it will show only the county and total diff. The final code line is so that we will see only the top ten results. The repeating of the varaible at the bottom is just so we will get a tibble below.
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer**
This code gave us a basic bar chart. It is showing us the total diff by county. The county names are not formatted well though and overlap along the x axis.
```{r}
maryland_top_counties %>%
  ggplot() +
...
**Answer**
The chart is easier to look at now that the bars have a better flow and the data is organized in a way that makes more sense. The bottom is still messy.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
The chart has been flipped now and the chart coordinates are flipped to the y-axis. We are now able to read the names of the counties.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
This code removed the gray background from the chart above.
...
**Answer**
This code gives us access to themes that we can use to liven up our graphics and make them more visually appealing.
...
**Answer** We now have a very eh blue background and our county names are smushed against the data descriptions. I know you mentioned this was a really populat style, but I am very much not a fan for some reason.
```{r}
maryland_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(County,TOTAL_DIFF), weight=TOTAL_DIFF)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
This code is cleaning up the data descriptions and giving them the names we set in the code.
```{r}
maryland_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(County,TOTAL_DIFF), weight=TOTAL_DIFF)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
This code gave us a two column tibble that includes contribution amounts and the date they were received.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
This gave us a very basic line chart. It doesn't have enough details yet to be super useful, but you can tell there is some sort of pattern.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
This chart adds specific dates to the x-axis, so we're now getting a better understanding of when these trends were taking place. It's still messy though and hard to read.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
This code spaced out the dates and made it much easier to digest. I could now confidently look at this graph and feel like I knew what was going on for the most part.
```{r}
md_winred_by_date %>%
  ggplot() + 
  geom_line(aes(x=date, y=total_contributions)) + 
