...
**Answer**
This code gave us the precinct id and name, polling id, school info, and location shape, area, length, etc for Prince George's Co.
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This installed a package for us, there was no immediate output.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** This gave us a glimpse of the county code. We now know there are 3,234 rows and 18 columns. It also showed us how all of the columns are identified.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This created a md_counties variable that is filtered for only STATEFP's that are 24.
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This created a basic grey map of Prince George's County based on the variable created in the code above.
```{r}
md_counties %>%
  ggplot() +
  geom_sf() +
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** This code gives us a more detailed map of PG County precincts.
...
**Task** Run the following code. Describe the output in the space below.
**Answer** 
This code gave us a table that includes precinct, the total number of polls, early votes, absentee votes, provisional votes and eligible voters for PG County.
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This code jointed the pg_turnout with pg_precincts.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Our new map based on the join we did in the code above has color! 
```{r}
ggplot() +
...
**Answer**
This code added our census key so we have access to that data now.
...
library(janitor)
library(tidycensus)
census_api_key("3e4cddbcf0e9838bf5e2b94705b5b102454010eb")
```
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This code adds in the new eligible voter data and created a tibble showing MD counties, their voter parties and their related numbers, as well total numbers.
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This pulled in the ACS data from tidycensus and created a tibble displaying that.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This code adds a geometry=TRUE line to the code above that now gives us the GEOID for the counties and their multipolygon info.
...
This code shows is the md_county_population data. I'm not sure what the bottom part is doing.
```{r}
md_county_population
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This cleaned up some of the md_county_population data so that we can get read to join it. It cleans and selects what we want for the next steps.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Since the data was cleaned above, in this code we are able to left join to the voter_by_county data.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This code cleans up two of the counties so they are uniform.
```{r}
md_county_population <- md_county_population %>%
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This code joined the datasets and gave us a description of the code, the top 10 counties and their multipolygon information.
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This code gave a map of Maryland based on the 10k voter dataset. It does have a key and the colors of the map represent voter numbers.
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This code changed the color of map, making it easier to look at and tell differences.
...
**Answer**
This code changed up the colors/lines slightly so that it is easier to look at for the little differences.
...
**Task** Run the following code. Describe the output in the space below.
**Answer** 
This gives us the percentage change for unaffiliated voters.
...
**Answer** 
This code adds the percentage of unaffiliated voters to the map.
```{r}
ggplot() +
