...
**Answer**
This gave us a four column table with 999 rows of Cardin's releases. The data published, title, url and press release text are all included.
```{r}
...
**Answer**
This lists out each entry for words or punctuation that might show up in different styles throughout the text.
```{r}
...
**Answer**
This gave us a very long list of all the words used in the press releases. If I'm reading it correctly, it collected 599k tokens.
...
**Answer**
This gave us a deeply profound and moving graph of the top 25 words, with the first being (drum roll please) "the". It is truly not interesting at all, it's all filler words.
...
**Answer**
This actually is more interesting! We now have a tibble of the most frequently occuring words, with the top five being health, support, act, funding and program. While not totally surprising that some of the top are things like support and funding (big things for a politician) I did find in interesting that health was the top hit. It makes me want to know more about his stance on health related issues and what he supports.
...
**Answer**
The results were pretty similar, but a noticable difference was in 2021 where the fifth most used word was Covid. It makes sense given that the pandemic was still in a newer and more covered spot in the media and politics at that time. They both over had health, support and funding in their top 5 as well. There were a lot of Covid related terms in 2021, and none in 2022, which was surprising.
```{r}
unique_words_2021 %>%
  anti_join(stop_words) %>%
  group_by(word) %>%
...
**Task** Run the following code and describe the results.
**Answer**
We are now looking at a list of ranked for frequency two word phrases. Covid-19 is the top phrase now, which makes sense now that we have the ability to search for a two word phrase, but it surprising a little that  just covid didn't even show in the top ten in the previous search. This new code also brought in new phrases like the Chesapeake Bay and the names of state politicians.
...
**Task** Run the following code and describe the results.
**Answer**
This gave us two word phrases to rank for frequency for 2021. Project, location, Covid-19, health care, Chesapeake Bay and human rights now take the top 5 spots. It is interesting to see the new additions like Chesapeake Bay. It was surprising to see project location knock down covid-19 a spot.
```{r}
releases %>%
  filter(date >= '2022-01-01') %>%
  unnest_tokens(bigram, text, token = "ngrams", n = 2) %>%
...
At first glance I thought most looked fine, but the more you think about it the more context for all of them becomes importance. "Issue" is deemed negative, but what about when you're just talking about something that has to be done? It's an "issue at hand" which is not necessarily negative. Or lead being positive. If something "lead" to a death or a major crisis, is it still positive? "Support" was the top phrase both years, which makes sense and in this context probably was positive. But like with examples mentioned before, what if it was supporting something bad or contreversial? 
