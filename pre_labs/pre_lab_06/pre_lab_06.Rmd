...
```{r}
voters_by_county <- read_csv("active_voters_by_county.csv")
...
**Answer** 
There are a few NA columns and the precinct column is not properly named. The turnout column is also a character field.
...
There are still N/A spots in the cast column. 
```{r}
camden_2022 <- read_csv("tabula-camden_county_election_results.csv") %>% clean_names()
...
**Answer** 
There are 21 rows instead of 23 rows like in Task 6.
```{r}
