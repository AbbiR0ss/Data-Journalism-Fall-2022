...
**Answer**
It says "One or more parsing issues, see `problems()` for details."
```{r}
...
**Answer**
There were 1,640 rows. I think this problem is because of the characters/numbers in the expected column.
...
**Answer**
The district field is a number field, the precinct field is a character field and the mail field is also a number field. 
```{r}
...
**Answer**:
There are 402,345 results that are NA. This could mean there is something that made the numbers unreadable for those results. It could have been a character instead of a number.
```{r}
...
**Answer**
There are 12 registrations for Jan. 1900. That could have been some type of human error when registering, but is most likely some sort of place holder number put in by the people gathering the data and working with it. The first 20 results overall seem like there could be some errors or place holder numbers that got left over.
```{r}
yadkin_voters %>% 
...
**Answer**
I would be interested to know what legacy data meant and why it was the second highest count. I also think it could be interesting to explore the information on those with felony conviction and who have completed there sentences. Many states don't allow felons to vote.
```{r}
...
**Answer**
The amount field is a "character" data type instead of a "number" like it should be.
```{r}
glimpse(conowingo)
```
...
**Answer**
The last_name category is in all caps while the other name categories are not, the zipcodes and formatted differently for some, and the city name is spelled/capitalizzed differently in multiple spots. The amount field is also a character field, not a number.
...
**Answer**
It adds an 'x' to the front of its name.
...
**Answer**
The error says "Problem while computing `total_amount = sum(amount)`" This makes me think there is an error with the amount column, possibly how it is characterized, and that's why it's unreadable.
...
**Answer**
It is now a <dbl> or number data type.
...
**Answer**
There are two duplicate rows for donor Derrick Hamilton.
```{r}
cleaned_conowingo %>% 
  get_dupes()
```
...
**Answer**
The zipcodes are all now uniform and have 5 digits.
...
I'm assuming this was supposed to say Conowingo instead of Arnold, I found two mispellings of Conowingo after running the code.
...
**Answer**
I found one mispelling of Conowingo.
